...
describe('DeepSeekProvider', () => {
  ...
  describe('constructor and setup', () => {
    ...
    test('getDisplayName returns full name', () => {
      expect(provider.getDisplayName()).toBe('DeepSeek');
    });
    ...
    test('setApiKey stores key', () => {
      provider.setApiKey('sk-deepseek-test');
      expect(provider.apiKey).toBe('sk-deepseek-test');
    });
  });
  ...
  describe('translate()', () => {
    ...
    test('calls DeepSeek API with correct params', async () => {
      provider.setApiKey('sk-deepseek-test');
      ...
      expect(options.headers['Authorization']).toBe('Bearer sk-deepseek-test');
  ...
  describe('validateKey()', () => {
    test('returns valid for 200 response', async () => {
      ...
      const result = await provider.validateKey('sk-deepseek-valid');
    ...
    test('returns invalid for 401 response', async () => {
      ...
      const result = await provider.validateKey('sk-deepseek-invalid');
    ...
    test('returns error for other HTTP errors', async () => {
      ...
      const result = await provider.validateKey('sk-deepseek-test');
    ...
    test('sends minimal request for validation', async () => {
      ...
      await provider.validateKey('sk-deepseek-test');
