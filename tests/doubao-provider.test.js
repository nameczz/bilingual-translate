...
import { DoubaoProvider } from '../src/services/doubao-provider.js';
...
describe('DoubaoProvider', () => {
  ...
  describe('constructor and setup', () => {
    ...
    test('getDisplayName returns full name', () => {
      expect(provider.getDisplayName()).toBe('Doubao (豆包)');
    });
    ...
    test('setApiKey stores key', () => {
      provider.setApiKey('sk-doubao-test');
      expect(provider.apiKey).toBe('sk-doubao-test');
    });
  });
  ...
  describe('translate()', () => {
    ...
    test('calls Doubao API with correct params', async () => {
      provider.setApiKey('sk-doubao-test');
      ...
      expect(options.headers['Authorization']).toBe('Bearer sk-doubao-test');
      expect(options.headers['Content-Type']).toBe('application/json');

      const body = JSON.parse(options.body);
      expect(body.model).toBe('doubao-1.5-pro-32k');
  ...
  describe('validateKey()', () => {
    test('returns valid for 200 response', async () => {
      ...
      const result = await provider.validateKey('sk-doubao-valid');
    ...
    test('returns invalid for 401 response', async () => {
      ...
      const result = await provider.validateKey('sk-doubao-invalid');
    ...
    test('returns error for other HTTP errors', async () => {
      ...
      const result = await provider.validateKey('sk-doubao-test');
    ...
    test('sends minimal request for validation', async () => {
      ...
      await provider.validateKey('sk-doubao-test');
