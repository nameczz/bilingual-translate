...
import { QwenProvider } from '../src/services/qwen-provider.js';
...
describe('QwenProvider', () => {
  ...
  describe('constructor and setup', () => {
    ...
    test('getDisplayName returns full name', () => {
      expect(provider.getDisplayName()).toBe('Qwen (通义千问)');
    });
    ...
    test('setApiKey stores key', () => {
      provider.setApiKey('sk-qwen-test');
      expect(provider.apiKey).toBe('sk-qwen-test');
    });
  });
  ...
  describe('translate()', () => {
    ...
    test('calls Qwen API with correct params', async () => {
      provider.setApiKey('sk-qwen-test');
      ...
      expect(options.headers['Authorization']).toBe('Bearer sk-qwen-test');
  ...
  describe('validateKey()', () => {
    test('returns valid for 200 response', async () => {
      ...
      const result = await provider.validateKey('sk-qwen-valid');
    ...
    test('returns invalid for 401 response', async () => {
      ...
      const result = await provider.validateKey('sk-qwen-invalid');
    ...
    test('returns error for other HTTP errors', async () => {
      ...
      const result = await provider.validateKey('sk-qwen-test');
    ...
    test('sends minimal request for validation', async () => {
      ...
      await provider.validateKey('sk-qwen-test');
