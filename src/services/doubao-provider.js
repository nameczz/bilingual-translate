/**
 * Doubao (豆包) translation provider.
 * Uses Volcengine Ark OpenAI-compatible chat completions API.
 */
import { TranslationProvider } from './provider.js';

const DOUBAO_API_URL = 'https://ark.cn-beijing.volces.com/api/v3/chat/completions';
const MODEL = 'doubao-1.5-pro-32k';
...
export class DoubaoProvider extends TranslationProvider {
  constructor() {
    super('Doubao');
    this.apiKey = null;
  }
  ...
  /**
   * Translate a single text using Doubao API.
   */
  async translate({ text, sourceLang, targetLang }) {
    if (!this.apiKey) {
      throw new Error('API key not configured. Please set your Doubao API key in the extension settings.');
    }
...
- Keep proper nouns, brand names, and technical terms as-is when appropriate`;
  ...
  /**
   * Call the Doubao chat completions API (OpenAI-compatible format).
   */
